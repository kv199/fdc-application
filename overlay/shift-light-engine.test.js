...
test('confirms the earliest of three clean better real upshifts', () => {
  ...
  assert.deepEqual(
    { status: gear.status, rpm: gear.shiftRpm, confirmations: gear.confirmingCount },
    { status: 'optimal', rpm: 9500, confirmations: 3 }
  )
})

test('uses redline without purple before a real better-shift candidate exists', () => {
  const learner = new ShiftLightLearner(getShiftLightCarKey(frame()))
  const state = learner.update(frame({ rpm: 11000 }))
  assert.equal(state.shiftRpm, null)
