...
test('Events keeps the existing navigation and exposes the create/detail flow', () => {
  ...
  assert.match(settingsJs, /function copyEventId\(eventId\)/)
  assert.match(settingsJs, /clipboard\.writeText\(value\)/)
  assert.match(settingsJs, /id\.textContent = `#\$\{event\.id\}`/)
  assert.match(settingsJs, /copy\.setAttribute\('aria-label', `Copy event ID \$\{event\.id\}`\)/)
  ...
  assert.match(settingsCss, /\.events-card__copy\s*\{[\s\S]*position:\s*absolute/)
