...
test('Configuration exposes Driver Analysis second in the primary tab order', () => {
  ...
  assert.match(settingsJs, /call\('delete_driver_analysis_session', \{ sessionId: id \}\)/)
  assert.match(settingsJs, /Delete this Driver Analysis recording and its saved telemetry\?/)
...
test('Driver Analysis history cards expose collapsible details with summary line and most-frequent pattern', () => {
  assert.match(settingsJs, /const expandedDriverAnalysisEntries = new Set\(\)/)
  assert.match(settingsJs, /expandedDriverAnalysisEntries\.delete\(String\(id\)\)/)
  assert.match(settingsJs, /formatPatternSummary.*entry\.stats/)
  assert.match(settingsJs, /label\.textContent = 'MOST FREQUENT'/)
  assert.match(settingsJs, /instruction\.textContent = patternSummary\.text/)
  assert.match(settingsJs, /formatSummaryLine.*entry\.stats/)
  assert.match(settingsJs, /summary\.className = 'driver-analysis-history-row__summary'/)
  assert.match(settingsJs, /formatStatsRows.*entry\.stats/)
  assert.match(settingsJs, /if \(statsRows\.length > 0\)/)
  assert.match(settingsJs, /detailsButton\.textContent = isExpanded.*'HIDE'.*'DETAILS'/)
  assert.match(settingsJs, /detailsButton\.setAttribute\('aria-expanded'/)
  assert.match(settingsJs, /expandedDriverAnalysisEntries\.has\(entryId\)/)
  assert.match(settingsJs, /expandedDriverAnalysisEntries\.add\(entryId\)/)
