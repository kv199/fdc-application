...
test('Events keeps the existing navigation and exposes the create/detail flow', () => {
  ...
  assert.match(settingsHtml, /id="events-detail-metadata"[^>]+aria-label="Event metadata"/)
  assert.doesNotMatch(settingsHtml, /id="events-detail-hint"|id="events-detail-card"/)
  ...
  assert.match(settingsJs, /events-detail-view__metadata-row/)
  assert.match(settingsJs, /\['MODE', eventModeLabel\(event\.mode\), true\]/)
  assert.match(settingsJs, /\['ROUTE TYPE', eventRouteLabel\(event\.routeType\), false\]/)
  assert.match(settingsJs, /\['CLASS', eventText\(event\.eventClass\)\.toUpperCase\(\) \|\| 'ANY', false\]/)
  assert.match(settingsJs, /if \(event\.notes\) fields\.push\(\['NOTES', event\.notes, false\]\)/)
