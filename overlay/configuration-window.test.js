...
test('Configuration exposes Driver Analysis second in the primary tab order', () => {
  ...
  assert.match(settingsJs, /Delete this Driver Analysis recording, all its cars, and saved telemetry\?/)
  assert.match(settingsJs, /phase === 'waiting'/)
  assert.match(settingsJs, /phase === 'finalizing'/)
  ...
  assert.doesNotMatch(overlayHtml, /asphalt-coach|driver-coach|coach-layout/)
...
test('Events keeps the existing navigation and exposes the create/detail flow', () => {
  ...
  assert.match(settingsJs, /call\('load_event_absolute_best', \{ eventId: nativeEventId \}\)/)
  assert.match(settingsJs, /eventsDetailAbsoluteBest\.textContent = formatRunTime\(currentEventAbsoluteBestMs\)/)
...
test('Events expose deterministic persisted list sorting and in-memory run sorting', () => {
  ...
  assert.match(settingsHtml, /<span class="event-run-row__header-label">LAPS<\/span>/)
  assert.match(settingsHtml, /data-run-sort="date"/)
  const bestHeader = settingsHtml.indexOf('data-run-sort="best"')
  const lapsHeader = settingsHtml.indexOf('event-run-row__header-label">LAPS')
  const dateHeader = settingsHtml.indexOf('data-run-sort="date"')
  assert.ok(bestHeader < lapsHeader && lapsHeader < dateHeader)
  assert.match(settingsJs, /const lapCount = run\.laps\.length/)
  assert.match(settingsJs, /\['LAPS', lapCount\]/)
  assert.match(settingsJs, /Open run \$\{run\.id\}, \$\{lapCount\} \$\{lapCount === 1 \? 'lap' : 'laps'\}/)
...
test('Event lap rows expose trace details and time-weighted pedal statistics', () => {
  ...
  assert.match(settingsJs, /events-lap-detail__trace events-lap-detail__trace--\$\{pedal\}/)
  assert.match(settingsJs, /traceMapLayerSelection/)
  assert.match(settingsJs, /globalThis\.EventTraceMap\?\.nextLayerSelection/)
  assert.match(settingsJs, /detail\.append\(renderTraceMap\(lap\.tracePoints \|\| \[\], lap\.timeMs\)\)/)
  assert.doesNotMatch(settingsJs, /function renderTraceStats/)
  assert.match(settingsJs, /globalThis\.EventTraceMap\?\.pedalState/)
  assert.match(settingsJs, /globalThis\.EventTraceMap\?\.groupRuns/)
  ...
  assert.match(settingsCss, /\.events-lap-detail\s*\{[\s\S]*grid-template-columns: minmax\(0, 1fr\);/)
...
test('Driver Analysis history cards expose collapsible details with summary line and most-frequent pattern', () => {
  assert.match(settingsJs, /const expandedDriverAnalysisRecordings = new Set\(\)/)
  assert.match(settingsJs, /const expandedDriverAnalysisCarRows = new Set\(\)/)
  assert.match(settingsJs, /delete_driver_analysis_recording/)
  assert.match(settingsJs, /DriverAnalysisHistory\?.groupRecordings/)
  assert.match(settingsJs, /DriverAnalysisHistory\?.recordingSummary/)
  assert.match(settingsJs, /DriverAnalysisHistory\?.carLabel/)
  assert.match(settingsJs, /formatPatternSummary.*session\.stats/)
  assert.match(settingsJs, /textContent = 'MOST FREQUENT'/)
  assert.match(settingsJs, /patternSummary\.text/)
  assert.match(settingsJs, /formatSummaryLine.*session\.stats/)
  assert.match(settingsJs, /summary\.className = 'driver-analysis-history-row__summary'/)
  assert.match(settingsJs, /formatStatsRows.*session\.stats/)
  assert.match(settingsJs, /if \(statsRows\.length > 0\)/)
  assert.match(settingsJs, /detailsButton\.textContent = isRecordingExpanded.*'HIDE'.*'DETAILS'/)
  assert.match(settingsJs, /detailsButton\.setAttribute\('aria-expanded'/)
  assert.match(settingsJs, /expandedDriverAnalysisRecordings\.has\(recordingId\)/)
  assert.match(settingsJs, /expandedDriverAnalysisRecordings\.add\(recordingId\)/)
  assert.match(settingsJs, /expandedDriverAnalysisCarRows\.add\(sessionId\)/)
  assert.match(settingsJs, /driver-analysis-history-row__details-panel/)
  assert.match(settingsJs, /driver-analysis-history-row__cars-section/)
  assert.match(settingsJs, /driver-analysis-history-row__car-row/)
  ...
  assert.match(settingsCss, /\.driver-analysis-history-row__details-item-value[\s\S]*font-variant-numeric:\s*tabular-nums/)
  assert.match(settingsCss, /\.driver-analysis-history-row__summary\s*\{[\s\S]*margin-top:\s*8px/)
  assert.match(settingsCss, /\.driver-analysis-history-row__cars-section/)
  assert.match(settingsCss, /\.driver-analysis-history-row__car-row/)
  assert.match(settingsCss, /\.driver-analysis-history-row__drives-table/)
})
